 ...
 * client id/secret are fleet-service's own registered Keycloak client
 * (infra/keycloak/realm-export.json), not shared with any other service.
 */
@Global()
@Module({
  providers: [
      ...
      useFactory: () =>
        new PostingAuthorityClient(
          ...
          new M2MTokenClient({
            ...
            clientId: process.env.M2M_CLIENT_ID ?? 'fleet-service',
